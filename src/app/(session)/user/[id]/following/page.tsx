...
import { getUserFollowing } from "@/app/actions";
...
export default async function UserFollowing({ params }: { params: { id: string } }) {
  const following = await getUserFollowing(params.id)
    ...
    <main className="flex min-h-screen flex-col items-center">
      ...
      <div className="w-full md:max-w-[400px] grid grid-cols-1 mx-auto mt-4">
        <div className="px-4 md:px-8 md:top-16 md:h-full md:col-span-1 my-4 md:my-0 w-full justify-center">
          {
            following && following.map((user: { name: string, bio: string | null, id: string, avatar: string | null }) => {
              return (
